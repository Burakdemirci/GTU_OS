...
RAND_NUM ds 2		  ; Genarated random number veriable	
COUNT ds 2 			  ; Message box Count
...
loop_body:
	MVI A, RAND_INT    ; Create random int (0-255)
	call GTU_OS        ; B <- randomNumber 
	MOV A, B           ; A = B 
	STA RAND_NUM	   ; assign random number to veriable 
	;;;;;;;;; JMP wait_func	   ; Call wait function  		
critical_region:
	LDA RAND_NUM	    ; Load the created random number 
	MOV D,A             ; Set D
	MVI B,1	   			; ID of MB	
	MVI C,3   			; Address off message box counter 
	MVI A,INSERT_MEM 	; Call insert system call 
	call GTU_OS	
